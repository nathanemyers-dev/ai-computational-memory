...
| advisory exposure | A boundary condition in which a packet, transformation, or access pattern would place computational memory within a regulated advisory domain, even if advisory‑state has not yet fully formed. Advisory exposure occurs when retained structures could reasonably be interpreted as guidance, suitability, or decision‑shaping input. The supervised persistence contract enforces rules that automatically reject any operation that would create advisory exposure, ensuring that continuity remains non‑advisory and that retained packets cannot form advisory‑state. |
...
| directional content | Output that implies a direction, course of action, or preferred path, even if it does not explicitly recommend a specific choice. Directional content shapes how a user moves forward and is treated as advisory activity. Transformation rules and supervised persistence must prevent retained packets from producing directional content so that computational memory remains within the non‑advisory domain. |
...
| evaluative content | Output that scores, ranks, compares, or judges options, users, or scenarios in a way that influences decisions or implies suitability. Evaluative content is classified as regulated guidance and therefore prohibited within computational memory. Transformation boundaries ensure that retained packets cannot generate evaluative content, preserving the separation between continuity and decision systems. |
